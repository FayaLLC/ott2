...
def generate_web_view_url(
    ...
    for ch in channels:
        ...
        unclean_category = ch.event_cat.capitalize() if ch.event_cat else "Sportzx"

        category = re.sub(r"[^\w\s\-\:\(\)\,\.\']", " ", unclean_category).strip()
        ...
        if event_name:
            if event_name not in group:
                group[event_name] = {"event_name": event_name, "streams": []}
                if ch.event_time and ch.event_time.strip():
                    group[event_name]["event_time"] = utc_to_local(ch.event_time)
            group[event_name]["streams"].append(stream_entry)
        else:
            # Use a unique placeholder key for ungrouped entries within the category
            key = f"__ungrouped_{len(group)}__"
            entry = {"streams": [stream_entry]}
            if ch.event_time and ch.event_time.strip():
                entry["event_time"] = ch.event_time  # utc_to_local(ch.event_time)
            group[key] = entry

    return [
        {"group": category, "items": list(events.values())}
...
if __name__ == "__main__":
    ...
    with open("web-view-urls.json", "w") as fh:
        json.dump(web_views, fh, indent=4, sort_keys=True)
